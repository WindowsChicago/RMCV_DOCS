...
2.在`SELECT COMPONENTS`时勾选`Additional icons`即可产生桌面图标，选编辑器时无所谓或者按需选择

3.配置初始分支名称，Git 默认选项为 `Let Git decide`，该选项通常会将初始分支名称设置为 `master`。如果希望使用 `main` 作为默认分支名称，请选择 `Override the default branch name for new repositories`（覆盖新仓库的默认分支名称）单选按钮，并在文本框中输入 `main`，完成配置后，点击`Next`继续安装
...
先打开资源管理器并导航到 `C:\Users\<你的用户名>\.ssh`。

接着在 `.ssh` 文件夹下，新建一个文件，命名为 `config`（没有扩展名）
